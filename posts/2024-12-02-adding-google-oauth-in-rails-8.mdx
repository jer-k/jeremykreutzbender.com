...
route. For `scope`, we're requesting the ability to use the Open ID APIs to access to the user's email address and
profile information. We're also passing an optional parameter for `access_type` and setting it to `online` because we don't
need access to the Google APIs when the user isn't active on the application. Finally, `response_type` is added for us in
`#authorize_url` and the value is `code`. The url we end up redirecting to looks like this.
