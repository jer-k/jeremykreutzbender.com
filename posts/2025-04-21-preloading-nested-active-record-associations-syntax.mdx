...
[ActiveRecord](https://guides.rubyonrails.org/active_record_basics.html) query
...
If this looks unfamiliar to to you, let's set up the context.
...
is at the bottom. The syntax of the query is that that for all the `User` records we look up, we also want to load
...
```ruby
# N+1 problem. We query for the Team and Organization record on every loop
...
preload multiple `Book` associations.

Now that we understand different approaches to the preloading syntax, let's revisit syntax of our original query let's
revisit it quickly and rewrite it.
...
All we're doing in the original omitting the array syntax around `:organization` because we're only loading a single
association.
...
the source of where I learned this syntax. The answer shows some examples of how build up a structure of extremely
nested associations which shouldn't look unfamiliar now that we've covered the basics here. Hopefully this helps
the next time you're writing or reviewing some ActiveRecord code!
