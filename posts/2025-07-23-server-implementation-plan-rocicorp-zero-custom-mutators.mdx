...
It's been a minute since I wrote anything about [Zero](https://zero.rocicorp.dev) but I've
finally gotten back to working on my project that is using it. A lot changed between the time
...
write-up or maybe even SDKs at some point but for the time being I hope this helps anyone else
who is excited about Custom Mutators and isn't building a fullstack Typescript app. What follows
is a lot of words and some psuedo-code examples to help solidify the ideas. Let's dive in!

## High Level Abstract

At a high level the flow of the Custom Mutators server implementation is going to be that
we receive an HTTP request with a list of mutations that need to be run on our server. There
...
* Process the list of muations and capture the results
...
that that I hope makes logical sense from an implementation standpoint. Remember that there
...
requests sent from Zero. This endpoint needs to set up to receive a `POST` request and respond back
...
is being used. The single requirement for the endpoint is that is accepts a `POST` request. I set up
...
start off with the `AUTHORIZATION` header. Zero includes the JWT that we're using on the client in the
these requests so the first step in validating the request should be to decode the JWT and ensure that it is
...
integerity of the data being mutated.
...
In a similar fashion, and again optional, we can customize our Zero application ID by setting [ZERO_APP_ID](https://zero.rocicorp.dev/docs/zero-cache-config#app-id). In the request payload above, we saw `appID: "zero"`,
which is the default but we should configure this TODO FINISH
...
Here's are some real world examples from my implementation to help solidify the structure.
...
occurs, we still need return the data as a `200` response.
...
that the mutation payload we recieved in properly ordered and if
...
that the looping process can check against. If everything process
...
function processOneMutation(mutation):
  mutationId = {clientID: mutation.clientID, id: mutation.id}
  ...
  try:
    ...
    return {id: mutationId, result: {}}

  catch OutOfOrderError as e:
    return {id: mutationId, result: {error: "oooMutation", details: e.message}}

  catch AlreadyProcessedError as e:
    return {id: mutationId, result: {error: "alreadyProcessed", details: e.message}}

  catch any other error as e:
    return {id: mutationId, result: {error: "app", details: e.message}}
```
...
value when the syncing is finished. Distributed systems aren't
100% perfect so my assumption is that there is a scenario where mutations get bundled
together in the wrong order, sent to the server, we'll throw our `oooMutation` error,
send the data back to Zero and then Zero will fix the ordering and retry, but that's
just a guess. If both checks pass then we go ahead and update the `mutationID`, save
the record, and continue our processing.
...
`switch` statement with all the different mutators we implement. As the list
grows we might want to modularize this. The name of the mutation will look
...
the `AUTHORIZATION` header to ensure referential itegrity between the object
...
creating additional records on an audit log table or just emitting metrics.
...
sure to share your learnings with Zero community! I'll be following up this post with
